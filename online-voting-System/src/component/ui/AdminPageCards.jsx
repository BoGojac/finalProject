...
const AdminPageCards = () => {
  const navigate = useNavigate();
  const { fetchUsers, getTotalUsers, getActiveUsers } = useUserStore();

  useEffect(() => {
    fetchUsers(); // fetch when component mounts
  }, [fetchUsers]);
  ...
  const cards = [
    {
      ...
      description: (
        <div className="space-y-1">
          <p>Create, update, and access control users</p>
          <div className="flex justify-evenly text-xs text-gray-500 pt-2">
            <span>Total: {getTotalUsers()}</span>
            <span>Active: {getActiveUsers()}</span>
          </div>
        </div>
      ),
      icon: <Users className="text-blue-600" size={28} />,
