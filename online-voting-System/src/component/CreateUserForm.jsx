...
const CreateUserForm = ({ isOpen, onClose, onSuccess }) => {
  ...
  const {
    ...
    defaultValues: { gender: 'male', role: 'Admin' },
  });
  ...
  useEffect(() => {
    if (['Constituency Staff', 'Polling Station Staff'].includes(watchRole)) fetchConstituencies();
  }, [watchRole, fetchConstituencies]);
  useEffect(() => {
    if (watchRole === 'Polling Station Staff' && watchConstituency) fetchPollingStations(watchConstituency);
  ...
  const onSubmit = async (data) => {
    ...
    try {
       await axios.post('http://127.0.0.1:8000/api/userregister', {
      ...
      setError('root', { type: 'manual', message: error.response?.data?.message || 'Failed to create user' });
    }
  };
    ...
    <Modal title="Create New User" onClose={onClose}>
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex gap-4 mt-2">
            {['male', 'female'].map(gender => (
              <label key={gender}><input type="radio" value={gender} {...register('gender')} className="mr-1" /> {gender.charAt(0).toUpperCase() + gender.slice(1)}</label>
            ))}
          </div>
        </div>
        ...
        <div className="col-span-2 flex justify-end mt-6">
          <button type="submit" disabled={isSubmitting} className="bg-purple-600 text-white px-6 py-2 rounded hover:bg-purple-700">
